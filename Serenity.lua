...
function Serenity_UpdateIcons()
	...
	-- Create icons for reordering in the button menu
	local mana = UnitMana("player");

	for i=1, 9, 1 do
		if SerenityButtonTexture.Stones.Highlight[i] ~= SerenityConfig.Skin then
			getglobal(SerenityConfig.StoneLocation[i]):SetHighlightTexture("Interface\\Addons\\Serenity\\UI\\BaseMenu-02");
			texture = getglobal(SerenityConfig.StoneLocation[i]):GetHighlightTexture() 
	...
	-------------------------------------
 	if SerenityConfig.CountType == 0 then     -- None
		Sphere.display = "";
	elseif SerenityConfig.CountType == 1 then -- Drinks
	    Sphere.display = Count.Drink;
	elseif SerenityConfig.CountType == 2 then -- Mana Potion and Healing Potion
		Sphere.display = Count.ManaPotion.." / "..Count.HealingPotion;
	elseif SerenityConfig.CountType == 3 then -- HP Current
	...
	elseif SerenityConfig.CountType == 4 then -- HP Percent
		Sphere.color = SerenityTimerColor(((UnitHealth("player") / UnitHealthMax("player")) * 100));
	...
	elseif SerenityConfig.CountType == 5 then -- MP Current
		Sphere.display = tostring(UnitMana("player"));
	elseif SerenityConfig.CountType == 6 then -- MP Percent
		Sphere.display = floor(UnitMana("player") / UnitManaMax("player") * 100);
		Sphere.display = tostring(Sphere.display).."%";
	elseif SerenityConfig.CountType == 7 then -- Potion cooldown
	...
	if SerenityConfig.Circle == 3 then
		if SerenityPrivate.PotionCooldown > 0 then
	...
	if Serenity_ReorderTexture[1] == nil or ReorderRecheck then
		for i=1, 9, 1 do
			Serenity_ReorderTexture[i] = tostring(getglobal(SerenityConfig.StoneLocation[i]):GetNormalTexture():GetTexture());			
		end
		ReorderRecheck = false;
	end
...
--for i=1, 9, 1 do
--	ReorderTexture[i] = SerenityConfig.StoneLocation[i];
--end
Reorder = {}
...
function Reorder.UpdateOrder()
  for i=1,9 do
    ...
    getglobal("Reorder"..i.."Icon"):SetTexture(Serenity_ReorderTexture[i])
  end
...
function Reorder.Move_OnClick(dir)
  ...
  local temp = Serenity_ReorderTexture[id]
  local temp2 = SerenityConfig.StoneLocation[id]
  Serenity_ReorderTexture[id] = Serenity_ReorderTexture[id+dir]
  Serenity_ReorderTexture[id+dir] = temp
...
function Serenity_ButtonSetup()
	if SerenityConfig.SerenityLockServ then
		...
		if SerenityConfig.StonePosition[8] and Mount.Available then
...
function Serenity_SpellSetup()
	...
	for index=1, table.getn(SERENITY_SPELL_TABLE), 1 do
		if (index == 26) then -- For Polymorph
			if SERENITY_SPELL_TABLE[index].ID ~= nil then
				SERENITY_SPELL_TABLE[index].Length = SERENITY_SPELL_TABLE[index].Rank * 10 + 10;
			end
		end
		if (index == 21) then  -- For Frost bolt
			nameTalent, icon, tier, column, currRank, maxRank = GetTalentInfo(3,7);
			if SERENITY_SPELL_TABLE[index].ID ~= nil and SERENITY_SPELL_TABLE[index].Rank <=7 then
    			SERENITY_SPELL_TABLE[index].Length = ceil(4.5 + currRank + (SERENITY_SPELL_TABLE[index].Rank * 0.5));
   			else 
			   	SERENITY_SPELL_TABLE[index].Length = SERENITY_SPELL_TABLE[index].Length + currRank
			end
		end
		if (index == 16) then  -- for Fireball
			if SERENITY_SPELL_TABLE[index].ID ~= nil and SERENITY_SPELL_TABLE[index].Rank <= 3 then
				SERENITY_SPELL_TABLE[index].Length = 6;
			end
		end
		if (index == 14) then  -- For fireblast
			if SERENITY_SPELL_TABLE[index].ID ~= nil then
				_, _, _, _, currRank, maxRank = GetTalentInfo(2,7);
				SERENITY_SPELL_TABLE[index].Length = 8 - (currRank * 0.5);		
			end
		end
		if (index == 19) then   -- For frost nova
			if SERENITY_SPELL_TABLE[index].ID ~= nil then
				_, _, _, _, currRank, maxRank = GetTalentInfo(3,6);
				SERENITY_SPELL_TABLE[index].Length = 25 - (currRank * 2);
			end
		end
	end
...
--	if SERENITY_SPELL_TABLE[1].ID or SERENITY_SPELL_TABLE[2].ID then
--		MountAvailable = true;
--	else
...
function Serenity_Decursive(type)
	...
	if afflicted ~= nil then
		...
		if type == "Magic" and not UnitIsFriend("player","target") then
			CastSpell(SERENITY_SPELL_TABLE[7].ID, "spell")
		else
